...
def settings_logic(mocker):
    """
    Provides a logic-only container and mocks external dependencies.
    """
    # FIX: Use a dictionary for side_effect to handle repeated calls
    # in the new migration logic.
    mock_keyring_data = {}

    def get_password_side_effect(service, key):
        return mock_keyring_data.get(key)

    mocker.patch("keyring.get_password", side_effect=get_password_side_effect)
    mocker.patch(
        "keyring.set_password",
        side_effect=lambda s, k, v: mock_keyring_data.update({k: v}),
    )
    ...
    logic_container._keyring_data = mock_keyring_data
...
def test_load_settings(settings_logic):
    ...
    # ARRANGE: Populate the mock keyring data
    settings_logic._keyring_data.update(
        {
            "BRAZE_API_KEY": "key",
            "TRANSIFEX_API_TOKEN": "token",
            "BRAZE_REST_ENDPOINT": "endpoint",
...
def test_load_settings_with_disabled_options(settings_logic):
    ...
    settings_logic._keyring_data.update(
        {"BACKUP_ENABLED": "0", "AUTO_UPDATE_ENABLED": "0", "LOG_LEVEL": "Debug"}
    )
    settings_logic.load_settings()
...
def test_save_settings(settings_logic):
    ...
    assert settings_logic._keyring_data[KEY_BACKUP_ENABLED] == "1"
    assert settings_logic._keyring_data[KEY_AUTO_UPDATE] == "1"
...
def test_reset_settings(settings_logic):
    ...
    assert keyring.delete_password.call_count == len(MANAGED_SETTINGS_KEYS)
